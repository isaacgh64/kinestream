...
export default function TopMenu() {
  ...
  return (
    <header className="bg-blue-600 text-white py-8 shadow-lg">
      ...
      <Link to={(!state.logIn)?"/login":"/user"} className="bg-yellow-400 text-blue-900 px-4 py-2 rounded-lg font-bold hover:bg-yellow-500 transition duration-300 me-8" onClick={() => dispatch({type:"change-view",payload:{view_stream:true,type:"movie",title:""}})}>{(!state.logIn)?"Iniciar Sesión":"Mi Cuenta"}</Link>
    </div>
</header>
